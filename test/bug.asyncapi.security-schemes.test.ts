...
// ── type (applies to all scheme variants) ────────────────────────────────────

describe('asyncapi security scheme / type field', () => {
...
// ── apiKey fields: name, in ───────────────────────────────────────────────────

describe('asyncapi security scheme / apiKey fields (name, in)', () => {
...
describe('asyncapi security scheme / http fields (scheme)', () => {
...
// ── oauth2 fields: flows ──────────────────────────────────────────────────────
// AsyncAPI uses `availableScopes` (not `scopes`) inside flow objects.

const clientCredentialsFlow = {
  clientCredentials: {
    tokenUrl: 'https://example.com/token',
    availableScopes: { 'read:api': 'Read access' },
  },
};
...
describe('asyncapi security scheme / oauth2 fields (flows)', () => {
  // Note: AsyncAPI unifier synthesizes `flows: {}` for type:oauth2 even when absent,
  ...
  it('adding availableScopes to an existing flow is non-breaking', () => {
    const before = {
      ...BASE,
      components: {
        securitySchemes: {
      ...
      components: {
        securitySchemes: {
          Auth: {
            type: 'oauth2',
            flows: {
              clientCredentials: { availableScopes: { 'read:api': 'Read access' } },
            },
          },
        },
      },
